...
def setup_styles(doc, theme: Theme):
    body = float(theme.raw.get("body_size", 11))
    s = doc.styles
    ...
    s["Normal"].paragraph_format.line_spacing = 1.25
    for name, size, bold, color, before, after in [
        ...
        ("LC H1", 16, True, theme.safe("primary"), 18, 10),
        ("LC H2", 13, True, theme.safe("primary"), 14, 7),
        ("LC H3", 12, True, theme.safe("title_color"), 10, 5),
